...
import './PromotionSection02.css'

import iconPromocao from '../../assets/iconPromocao.png'
import imgMissaoVerdao02 from '../../assets/imgMissaoVerdao02.png'
import imgVantagemRei02 from '../../assets/imgVatagemReiTurnina02.png'
import imgMissaoBrasileirao02 from '../../assets/imgMissaoBrasileirao02.png'
...
export function PromotionSection02() {
  ...
  const handleActivateMission = () => {
    if (selectedPromo) {
      ...
      const target = promoId === 'p2-1' ? 50 : 20
      setTimeout(() => {
  ...
  return (
    <section className="promotion-section-02">
      <div className="promotion-section-02__header">
        <div className="promotion-section-02__title">
          <img src={iconPromocao} alt="" className="promotion-section-02__icon" />
          <span>Promoções</span>
        </div>
      </div>

      <div 
        className={`promotion-section-02__list ${isDragging ? 'promotion-section-02__list--dragging' : ''}`}
        ref={scrollRef}
      ...
      >
        {promotions.map((promo) => {
          ...
          return (
            <div 
              key={promo.id} 
              className={`promo-card-02 ${isActivated ? 'promo-card-02--activated' : ''}`}
              onClick={() => handlePromoClick(promo)}
            >
              <div className="promo-card-02__image-wrapper">
                <img src={promo.image} alt="" className="promo-card-02__image" />
              </div>
              <div className="promo-card-02__content">
                <div className="promo-card-02__header">
                  <span className="promo-card-02__type">
                    {isActivated && progress && promo.type === 'missao' ? (
                      <span className="promo-card-02__progress">
                ...
                <div className="promo-card-02__text">
                  <p className="promo-card-02__title">{promo.title}</p>
                </div>
              </div>
            </div>
          )
        ...
        >
          ...
          <div className="mission-info-section">
            ...
            <div className="mission-info-rows">
              ...
              <MissionInfoRow label="Valor Mínimo por Aposta" value={selectedPromo.id === 'p2-1' ? 'Pelo menos R$50' : 'Pelo menos R$20'} />
              <MissionInfoRow label="Odd Mínima por Aposta" value="3.00x" />
              <MissionInfoRow label="Valor do Bônus" value={selectedPromo.id === 'p2-1' ? '10' : '5'} />
